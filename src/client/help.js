...
export function view(ctrl, args, extras) {
	return m('div', [
		...
		m('div.business-table.col-sm-8',
			...
			m('div', 'It depends on the vehicle. Not all vehicles report their fuel level.'),
			...
			m('div', 'The duration, in the format of HH:MM:SS, is time spent with the ignition in the prior state. For example, if the status is "ign off", this means the ignition was turned off. The duration then shows how long the ignition ran (prior to being turned off). ' +
			...
			m('div', 'This shows the odometer and engine hours columns, as well as displaying more detailed informational messages from the trackers.'),
			m('h4', 'What is a harsh status?'),
			m('div', 'The tracking device detected what it was programmed to be harsh acceleration, braking, or turning.'),
