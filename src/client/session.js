...
export function view(ctrl) {
	...
	return m('div', [
		m('.row center-block', {
			...
			!state.user.username ? m('input.form-control', {
			...
			!state.user.username ? m('label', m('input[type=checkbox]', {
			...
			m('div', {
				...
				state.user.username ? m('button.btn btn-default', {
				...
				}, t('Log Out') + ' ' + state.user.username) : null,
