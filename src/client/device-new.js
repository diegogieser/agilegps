...
export function controller(args, extras) {
	...
	ctrl.save = function () {
        appState.saveDevice(ctrl.device)
        .then(function () {
			window.history.back();
        })
        .catch(catchhandler);
...
export function view(ctrl) {
	...
	function lastKey(key) {
		switch (key) {
			...
			case 'lastHeartbeat':
			return [t('Last seen heartbeat') + ' ', m('span', {
				style: {
					'font-size': '1.5em',
					'color': 'red'
				}
			...
			case 'lastACKBSI':
			return t('Last seen bearer setting (GSM data service) config acknowledgement:');
				...
				Object.keys(device).map(function(key) {
					if (key.indexOf('last') === 0) {
						...
						return m('div.form-group', [
							...
							m('.col-md-8', m('input[disabled][class=form-control]', {
								value: device[key].senddate ? moment(device[key].senddate).format('M/DD/YYYY h:mm:ss A') : moment(device[key]).format('M/DD/YYYY h:mm:ss A')
							}))
						])
					} else if (key === 'orgid') {
