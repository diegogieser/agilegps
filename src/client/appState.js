...
function loadOrgState(orgid) {
    ...
    .then(function (org) {
        ...
        let fleetp = Promise.resolve(fetch('/api/organizations/' + orgid + '/fleets', auth()))
        .then(function (response) {
            return validateResponse(response);
        })
        ...
        // org users
        let usersp = Promise.resolve(fetch('/api/organizations/' + orgid + '/users', auth()))
        .then(function (response) {
            return validateResponse(response);
        })
        .then(function (users) {
...
function loadVehicles(orgid) {
    NProgress.start();
    return Promise.resolve(fetch('/api/organizations/' + orgid + '/vehiclestatus', auth()))
    .then(function (response) {
        NProgress.inc();
        return validateResponse(response);
    })
...
function loadSiteAdminData() {
    ...
    return Promise.all([orgsp, usersp, devicesp, vehiclesp])
    .then(function () {
        NProgress.inc();
    });
}
...
module.exports.deleteUser = function (user) {
    ...
    return Promise.resolve(fetch(url, Object.assign(auth(), {
        method: 'DELETE'
    })))
    .then(function (response) {
        return validateResponse(response)
...
module.exports.saveFleet = function (fleet) {
    ...
    if (fleet.id) {
        return Promise.resolve(fetch('/api/organizations/' + fleet.orgid + '/fleets/' + fleet.id, put(fleet)))
        .then(function (response) {
            NProgress.inc();
            return validateResponse(response)
        })
        ...
        return Promise.resolve(fetch('/api/organizations/' + fleet.orgid + '/fleets/', post(fleet)))
        .then(function (response) {
            NProgress.inc();
            return validateResponse(response)
        })
...
function updateSelectedVehicleHistory() {
    ...
    let url = '/api/organizations/' + vehicle.orgid + '/vehiclehistory/' + vehicle.id
             + '?startDate=' + encodeURIComponent(state.startDate.toISOString())
             + '&endDate=' + encodeURIComponent(state.endDate.toISOString());
...
module.exports.selectVehicleByID = function (id) {
    ...
    if (lastStatus != null && lastStatus.d) {
        return selectDay(moment(lastStatus.d).startOf('day').toDate());
    }
}
...
module.exports.postVehicle = function(vehicle) {
    ...
    return Promise.resolve(fetch('/api/organizations/' + vehicle.orgid + '/vehicles/', post(vehicle)))
    .then(function (response) {
        NProgress.inc();
        return validateResponse(response);
    })
...
function updateEvents () {
    ...
    return Promise.resolve(fetch('/api/count/' + type + (type === 'rawevents' ? '?search=' + encodeURIComponent(state.search) : ''), auth()))
    .then(function (response) {
        NProgress.inc();
        return validateResponse(response);
    })
    ...
    .then(function () {
        let url = '/api/' + type + '/?page=' + state.page + '&pagesize=' + state.pagesize + '&search=' + encodeURIComponent(state.search)
        return Promise.resolve(fetch(url, auth()))
    })
    .then(function (response) {
        NProgress.inc();
