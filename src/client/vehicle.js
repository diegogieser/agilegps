...
export function controller(args, extras) {
	...
	ctrl.selectDays = function () {
		...
		appState.selectDays(ctrl.startDate(), moment(ctrl.endDate()).add(1, 'day').toDate());
		// }
	}
	...
	ctrl.speedChange = function (ev) {
		...
		switch (val) {
			case 0: animation.setSpeed(400);
			break;
			case 1: animation.setSpeed(250);
			break;
			case 2: animation.setSpeed(100);
			break;
			case 3: animation.setSpeed(50);
			break;
			case 4: animation.setSpeed(35);
			break;
			case 5: animation.setSpeed(20);
			break;
			case 6: animation.setSpeed(15);
			break;
			case 7: animation.setSpeed(10);
			break;
			case 8: animation.setSpeed(5);
			break;
			case 9: animation.setSpeed(1);
			break;
			default: animation.setSpeed(0);
			break;
		}
	}
	...
	return m('div', [
		m('.business-table', [
			m('div',
				...
				m('button.btn btn-xs btn-primary btn-pad', {
					onclick: function () {
						ctrl.firstRowClicked = false;
						appState.update();
					}
				...
				m('a', {
					href: '/api/organizations/'
					 + state.selectedOrg.id + '/vehiclehistory/'
					 + state.selectedVehicle.id + '?format=excel'
					 + '&latlong=' + state.showLatLong
					 + '&rollupStationaryEvents=' + ctrl.rollup()
					 ...
					 + '&calculateDistanceBetween=' + ctrl.calculateDistanceBetween()
					 + '&reverse=' + (ctrl.reverseOrder() ? 'true' : 'false')
				...
				m('br'),
				m('.nowrap', [
					 m('label.padrt', m('input[type=checkbox]', {
						onclick: function() {
					...
					!advancedUI ? null : m('label.padrt', m('input[type=checkbox]', {
						onclick: function() {
					...
					state.user.isAdmin || advancedUI ? m('label.padrt', m('input[type=checkbox]', {
						onclick: function() {
							ctrl.rollup(this.checked);
							ctrl.recalculateAdjustedVehicleHistory();
						},
						checked: ctrl.rollup()
					}), t('Rollup idling & parked')) : null,

					(state.user.isAdmin) ? [m('label', m('input[type=checkbox]', {
						onclick: function() {
				...
				!advancedUI ? null : m('label', 'Showing ' + ctrl.adjustedVehicleHistory().length + (ctrl.vehiclehistory().length === 1 ? ' event' : ' events') + (ctrl.vehiclehistory().length === 0 ? ' (Try a different date range?)' : '')),
				m('table.table table-bordered table-striped', {
					...
					m('tbody',
					ctrl.adjustedVehicleHistory().length < 1 ? m('div', t('No vehicle history for this day')) :
					ctrl.adjustedVehicleHistory().map(function(item, index) {
						return m('tr', {
							config: function (el, isInitialized) {
								if (!ctrl.firstRowClicked && item.la != null && item.lo != null) {
							...
							class: ctrl.highlightIgnition() && (item.cmd === 'IGN') || ctrl.highlightStarts() && item.statusOverride === 'Start' ? 'highlight-igniton' : '',
							onclick: function(ev) {
								ctrl.clickItem(item);
							},
							key: item.id,
							style: j2c.inline({
								'background-color': item.id === ctrl.selecteditem().id ? '#FEE0C6' : ''
							})
						}, [
							...
							state.verbose ? m('td', item.b ? t('Buffered') : t('Yes')) : '',
							state.verbose ? m('td', item.bp) : '',
							m('td', helpers.getAccuracyAsImg(item.g)),
							ctrl.rawData() ? m('td', m('pre', {
								// if you click the raw message, reformat it nicely
								onclick: function (el) {
									el.target.innerText = JSON.stringify(item, null, 4);
								}
