    		...
    		m('table.table-condensed table-bordered table-striped dataTable', [
    			...
    			m('tbody', _.map(Object.keys(args.result().vehicles), function(vid) {
    				return args.result().results[vid].length > 0 ? [
    					m('tr', m('td[colspan=19].group', args.result().vehicles[vid].name)),
    					_.map(args.result().results[vid], function(result) {
    						...
    						return m('tr', [
                                ...
                                m('td', result.obd.malfunction ? result.obd.diagnosticTroubleCodes.length + '⚠' : ''),
                                m('td', result.obd.diagnosticTroubleCodes.map(function (code) {
                                    return [m('a', {
                                ...
                                m('td', result.obd.fuelLevelInput ? result.obd.fuelLevelInput + '%' : ''),
                                ...
                                m('td', result.obd.powermV ? result.obd.powermV /1000 + 'V' : ''),
                                m('td', result.obd.vin),
