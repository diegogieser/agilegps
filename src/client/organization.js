...
export function view(ctrl, args, extras) {
	...
	return m('.business-table', [
		...
		m('a.padrt', {
			href: '/api/organizations/' + state.selectedOrg.id + '/vehiclestatus?format=excel'
		...
		m('table.table.table-bordered.table-striped', sorts(state.impliedSelectedVehicles), [
			m('thead', {
				style: {
					cursor: 'pointer'
				}
			...
			state.impliedSelectedVehicles.map(function(vehicle) {
				...
				if (state.verbose) {
					if (vehicle.lastVerbose && vehicle.last) {
						if (new Date(vehicle.lastVerbose.d) >= new Date(vehicle.last.d)) {
				...
				return m('tr', {
					...
					style: {
						'transition': wasRecentlyUpdated(lastStatus.d) ? 'background-color 1s ease-in-out' : 'none',
						'background-color': vehicle.id === ctrl.selectedItem().id ? '#FEE0C6'
						 : wasRecentlyUpdated(lastStatus.d) ? 'yellow' : ''
					}
				}, [
					m('td.nowrap', vehicle.name),
					...
					!state.verbose ? '' : m('td.nowrap', hidenan(tomiles(lastStatus.m))),
					...
					!state.verbose ? '' : m('td.nowrap', lastStatus.b ? t('Buffered') : t('Yes')),
					!state.verbose ? '' : m('td.nowrap', lastStatus.bp ? lastStatus.bp + '%' : ''),
