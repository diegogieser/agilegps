...
let playPromise, history, progressEl, speed = 0, _controller;
...
function create (item) {
    ...
    google.maps.event.addListener(marker, 'click', ClickListenerFactory.create(marker, item, position));
...
store.subscribe(function () {
    TheMap.getReady().then(function () {
        ...
        if (lastSelectedVehicle === state.selectedVehicle) {
            ...
            if (!state.autoUpdate) {
                return;
            } else if (lastSelectedVehicleHistory === lastState.selectedVehicleHistory) {
                return;
            }
        }
...
function animateFrame (i) {
    ...
    if (!next) {
        console.warn('Vehicle animation frame ' + i + ', invalid position ' + JSON.stringify(next));
        return;
    }
    ...
    if (flightPlanCoordinates && flightPlanCoordinates.length == 2 && flightPlanCoordinates[0] != null && flightPlanCoordinates[1] != null) {
